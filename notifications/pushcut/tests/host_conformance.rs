...
const PLUGIN_ID: &str = "pushcut";
const PROVIDER_TYPE: &str = "pushcut";
const WASM_NAME: &str = "pushcut_notification.wasm";

/// The configuration Scryer would have resolved for this channel.
fn scripted_config(key: &str) -> Option<String> {
    match key {
        "notification_name" => Some("Scryer".to_string()),
        "api_key" => Some("pushcutkey".to_string()),
        _ => None,
    }
}
...
const EXPECTED_URL_PREFIX: &str = "https://api.pushcut.io/v1/notifications/Scryer";

static PLUGIN_WASM: OnceLock<PathBuf> = OnceLock::new();

#[test]
fn pushcut_release_wasm_conforms_to_the_notification_host_contract() {
...
fn assert_a_missing_required_setting_is_a_typed_error(wasm_path: &Path) {
    let script = Script {
        unset: vec!["notification_name".to_string()],
        ..Script::default()
    };
    let (mut store, plugin) = instantiate(wasm_path, script);
    let result = call_notification(
    ...
    assert!(
        error.public_message.contains("notification_name"),
