...
const PLUGIN_ID: &str = "apprise";
const PROVIDER_TYPE: &str = "apprise";
const WASM_NAME: &str = "apprise_notification.wasm";

/// The configuration Scryer would have resolved for this channel.
fn scripted_config(key: &str) -> Option<String> {
    match key {
        "server_url" => Some("https://apprise.test.invalid".to_string()),
        "configuration_key" => Some("scryer".to_string()),
        _ => None,
    }
}
...
const EXPECTED_URL_PREFIX: &str = "https://apprise.test.invalid/notify/scryer";
...
fn assert_a_missing_required_setting_is_a_typed_error(wasm_path: &Path) {
    ...
    assert!(
        error.public_message.contains("server_url"),
