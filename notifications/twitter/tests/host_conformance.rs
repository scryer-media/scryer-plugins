...
const PLUGIN_ID: &str = "twitter";
const PROVIDER_TYPE: &str = "twitter";
const WASM_NAME: &str = "twitter_notification.wasm";
...
const EXPECTED_URL_PREFIX: &str = "https://api.twitter.com/1.1/statuses/update.json";

static PLUGIN_WASM: OnceLock<PathBuf> = OnceLock::new();

#[test]
fn twitter_release_wasm_conforms_to_the_notification_host_contract() {
...
fn assert_a_missing_required_setting_is_a_typed_error(wasm_path: &Path) {
    let script = Script {
        unset: vec!["consumer_key".to_string()],
        ..Script::default()
    };
    let (mut store, plugin) = instantiate(wasm_path, script);
    let result = call_notification(
    ...
    assert!(
        error.public_message.contains("consumer_key"),
