...
const EXPECTED_URL_PREFIX: &str = "https://api.simplepush.io/send";

static PLUGIN_WASM: OnceLock<PathBuf> = OnceLock::new();

#[test]
fn simplepush_release_wasm_conforms_to_the_notification_host_contract() {
...
fn assert_a_missing_required_setting_is_a_typed_error(wasm_path: &Path) {
    ...
    assert!(
        error.public_message.contains("key"),
