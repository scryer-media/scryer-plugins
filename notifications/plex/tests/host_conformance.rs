...
const PLUGIN_ID: &str = "plex";
const PROVIDER_TYPE: &str = "plex";
const WASM_NAME: &str = "plex_notification.wasm";

/// The configuration Scryer would have resolved for this channel.
fn scripted_config(key: &str) -> Option<String> {
    match key {
        "base_url" => Some("http://plex.test.invalid:32400".to_string()),
        "update_library" => Some("true".to_string()),
        "section_ids" => Some("3".to_string()),
        "auth_token" => Some("plextoken".to_string()),
        _ => None,
    }
}
