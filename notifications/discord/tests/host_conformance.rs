...
const WASM_NAME: &str = "discord_notification.wasm";

/// The configuration Scryer would have resolved for this channel.
fn scripted_config(key: &str) -> Option<String> {
    match key {
        "webhook_url" => Some("https://discord.test.invalid/api/webhooks/1/token".to_string()),
        _ => None,
    }
}
...
const EXPECTED_URL_PREFIX: &str = "https://discord.test.invalid/api/webhooks/1/token";
...
fn assert_a_missing_required_setting_is_a_typed_error(wasm_path: &Path) {
    let script = Script {
        unset: vec!["webhook_url".to_string()],
        ..Script::default()
    };
    let (mut store, plugin) = instantiate(wasm_path, script);
    let result = call_notification(
