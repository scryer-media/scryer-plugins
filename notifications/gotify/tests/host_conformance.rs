...
const PLUGIN_ID: &str = "gotify";
const PROVIDER_TYPE: &str = "gotify";
const WASM_NAME: &str = "gotify_notification.wasm";

/// The configuration Scryer would have resolved for this channel.
fn scripted_config(key: &str) -> Option<String> {
    match key {
        "server" => Some("https://gotify.test.invalid".to_string()),
        "app_token" => Some("apptoken".to_string()),
        _ => None,
    }
}
...
fn assert_a_missing_required_setting_is_a_typed_error(wasm_path: &Path) {
    ...
    assert!(
        error.public_message.contains("server"),
