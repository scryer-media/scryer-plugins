...
Call `list_scopes` first. Use `scope = "/Users/jeremy/dev/scryer-media/scryer-plugins"` for this repo, or `scope = "workspace"` for cross-repo searches. MCP `pathPrefix` and `file` filters are repo-relative; never include `/Users/jeremy/dev/scryer-media` or `scryer-media` in those filters.
