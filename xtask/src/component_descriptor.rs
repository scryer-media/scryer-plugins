...
use self::scryer::indexer::host::{Host, HostWithStore, HttpRequest, HttpResponse, LogLevel, TransportError};
...
pub(crate) fn descriptor_from_component(wasm: &[u8]) -> Result<Option<PluginDescriptor>> {
    ...
    IndexerPlugin::add_to_linker::<DescriptorCtx, HasSelf<DescriptorCtx>>(&mut linker, |ctx| ctx)
        .map_err(|error| anyhow!("register indexer component descriptor host: {error:#}"))?;
    ...
    let describe = instance
        ...
        .map_err(|error| anyhow!("indexer component lacks a compatible describe export: {error:#}"))?;
    let (encoded,) = block_on(store.run_concurrent(async move |accessor| {
        describe.call_concurrent(accessor, ()).await
    }))
