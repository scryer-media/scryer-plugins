...
//!
//! ## The generator's polarity
//!
//! Whisper is the family's only `SubtitleProviderMode::Generator`, so two of
//! the eight assertion classes point the other way from every catalog
//! provider's suite: the operation that must work is `generate`, and the
//! operations that must answer in-band `Unsupported` are `search` and
//! `download`. `generate` reads its staged input from `wasi:filesystem`, so
//! this is also the only subtitle suite that preopens a directory — the stand-in
//! for what the host stages for a generator.
...
/// Whisper's API base is a compiled-in constant rather than a config field, so
/// the URL assertions pin the advertised base.
const OPENAI_API_BASE: &str = "https://api.openai.com/v1";
const TEST_API_KEY: &str = "test-api-key";
/// Where the host preopens the generator's staged input.
const GUEST_INPUT_DIR: &str = "/scryer/input";
const INPUT_FILE_NAME: &str = "Show.flac";
const AUDIO_BYTES: &[u8] = b"fLaC pretend this is a staged audio track";
...
#[test]
fn whisper_release_wasm_conforms_to_the_subtitle_host_contract() {
    let wasm_path = whisper_plugin_wasm();

    assert_artifact_is_a_component(&wasm_path);
    assert_world_conformance(&wasm_path);
    assert_describe_returns_a_generator_subtitle_descriptor(&wasm_path);
    assert_validate_config_reaches_the_host_services(&wasm_path);
    assert_generate_transcribes_the_staged_input(&wasm_path);
...
fn assert_artifact_is_a_component(wasm_path: &Path) {
    let bytes = std::fs::read(wasm_path).expect("read whisper plugin wasm");
    assert!(
...
fn assert_describe_returns_a_generator_subtitle_descriptor(wasm_path: &Path) {
    ...
    assert_eq!(descriptor["provider"]["provider_type"], "whisper");
    assert_eq!(
        descriptor["provider"]["capabilities"]["mode"], "generator",
        "whisper is the family's generator, not a catalog provider"
    );
...
/// The API key and the probe both travel over the one `host-call` import, and
/// the probe goes to the base URL the descriptor advertises.
fn assert_validate_config_reaches_the_host_services(wasm_path: &Path) {
    ...
    assert_eq!(
        http,
        &format!("http:{OPENAI_API_BASE}/models"),
        "the advertised API base must be used verbatim"
    );
}

/// The generator's one real operation: read the staged input off
/// `wasi:filesystem`, post it as multipart over host HTTP, and hand back the
/// transcript. This is the assertion that proves the file-staging half of the
/// contract survived the move to a component.
fn assert_generate_transcribes_the_staged_input(wasm_path: &Path) {
    let script = Script {
        http: Some(HttpScript {
            status: 200,
            body: TRANSCRIPT.to_vec(),
        }),
        ..Script::default()
    };
    let (mut store, plugin) = instantiate(wasm_path, script);

    let result = call_subtitle(&mut store, &plugin, generate_command());
    let PluginSubtitleCommandResult::Generate(PluginResult::Ok(response)) = result else {
        panic!("generate did not return a typed ok result: {result:?}");
    };

    use base64::Engine as _;
    let content = base64::engine::general_purpose::STANDARD
        .decode(response.content_base64)
        .expect("generate content is base64");
    ...
    assert!(
        calls
            .iter()
            .any(|call| call == &format!("http:{OPENAI_API_BASE}/audio/transcriptions")),
        "the transcription must be requested over host services: {calls:?}"
    );
}
...
fn assert_a_refused_host_capability_stays_in_band(wasm_path: &Path) {
    ...
    // is exactly what a Scryer refusing an egress policy would send. The
    // staged input is still readable, so this isolates the HTTP hop.
    let (mut store, plugin) = instantiate(wasm_path, Script::default());
...
/// The mirror of every catalog provider's "generate is unsupported" arm: a
/// generator has no catalog, the host never routes one here, and the arms
/// exist to answer rather than to trap.
fn assert_catalog_operations_are_unsupported_in_band(wasm_path: &Path) {
    let (mut store, plugin) = instantiate(wasm_path, Script::default());

    let search = call_subtitle(
        &mut store,
        &plugin,
        PluginSubtitleCommand::Search(search_request()),
    );
    let PluginSubtitleCommandResult::Search(PluginResult::Err(error)) = search else {
        panic!("search must report an in-band error: {search:?}");
    };
    assert_eq!(error.code, PluginErrorCode::Unsupported);

    let download = call_subtitle(
        &mut store,
        &plugin,
        PluginSubtitleCommand::Download(SubtitlePluginDownloadRequest {
            provider_file_id: "irrelevant".to_string(),
        }),
    );
    let PluginSubtitleCommandResult::Download(PluginResult::Err(error)) = download else {
        panic!("download must report an in-band error: {download:?}");
    };
    assert_eq!(error.code, PluginErrorCode::Unsupported);
}
...
fn generate_command() -> PluginSubtitleCommand {
    PluginSubtitleCommand::Generate(SubtitlePluginGenerateRequest {
        ...
        input: SubtitleGeneratorInputRef {
            path: PathBuf::from(GUEST_INPUT_DIR).join(INPUT_FILE_NAME),
            mime_type: "audio/flac".to_string(),
            duration_seconds: 1_400,
            size_bytes: AUDIO_BYTES.len() as i64,
...
/// The host directory that stands in for what Scryer stages for a generator.
fn staged_input_dir() -> PathBuf {
    INPUT_DIR
        .get_or_init(|| {
            let dir = PathBuf::from(env!("CARGO_TARGET_TMPDIR")).join("whisper-staged-input");
            std::fs::create_dir_all(&dir).expect("create the staged input directory");
            std::fs::write(dir.join(INPUT_FILE_NAME), AUDIO_BYTES)
                .expect("write the staged input file");
            dir
        })
        .clone()
}

fn whisper_plugin_wasm() -> PathBuf {
    PLUGIN_WASM
        .get_or_init(|| {
            ...
            let status = Command::new(cargo)
                ...
                .arg("wasm32-wasip2")
                .status()
                .expect("run cargo build for the whisper plugin");
            assert!(status.success(), "whisper plugin build failed: {status}");

            plugin_root.join("target/wasm32-wasip2/plugin-release/whisper_subtitle_provider.wasm")
        })
        .clone()
}
