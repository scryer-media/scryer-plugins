...
fn animetosho_release_wasm_conforms_to_the_subtitle_host_contract() {
    let wasm_path = animetosho_plugin_wasm();
...
fn assert_artifact_is_a_component(wasm_path: &Path) {
    let bytes = std::fs::read(wasm_path).expect("read animetosho plugin wasm");
    assert!(
...
fn assert_describe_returns_a_catalog_subtitle_descriptor(wasm_path: &Path) {
    ...
    assert_eq!(descriptor["id"], "animetosho-xyz-subtitles");
    ...
    assert_eq!(descriptor["provider"]["provider_type"], "animetosho-xyz");
    assert_eq!(
        descriptor["provider"]["capabilities"]["mode"], "catalog",
        "animetosho advertises itself as a catalog provider"
    );
...
/// The provider's configuration and its upstream probe both travel over the
/// one `host-call` import, and the API key it read is appended to the
/// configured base URL rather than to a compiled-in default.
fn assert_validate_config_reaches_the_host_services(wasm_path: &Path) {
    ...
    assert_eq!(
        http,
        &format!("http:{TEST_BASE_URL}/json/v1/search?q=naruto&limit=1&apikey={TEST_API_KEY}"),
        "the configured base URL must be used verbatim, with the key appended"
    );
}

/// Unlike the tsukihime pilot, this provider never opens the container: it
/// checks the XZ magic and hands the compressed attachment to Scryer as
/// `application/x-xz`. The assertion pins that — one host HTTP call, bytes
...
fn assert_download_streams_the_container_through_host_http(wasm_path: &Path) {
    ...
    assert_eq!(
        content,
        xz_fixture(),
        "the compressed attachment must reach Scryer byte-for-byte"
    );
    assert_eq!(response.format, "ass");
    assert_eq!(
        response.filename.as_deref(),
        Some("animetosho-xyz-619135-94001-eng.ass.xz")
    );
    assert_eq!(response.content_type.as_deref(), Some("application/x-xz"));
    ...
    assert!(
        ...
        "the attachment must be fetched over host services: {calls:?}"
    );
    assert!(
...
fn assert_a_refused_host_capability_stays_in_band(wasm_path: &Path) {
    ...
    assert!(
        error.public_message.contains("request failed"),
        ...
        let response = match request {
            PluginHostRequest::ConfigGet(request) => {
                ...
                let value = match request.key.as_str() {
                    ...
                    "site_url" => Some(TEST_SITE_URL.to_string()),
...
fn animetosho_plugin_wasm() -> PathBuf {
    PLUGIN_WASM
        .get_or_init(|| {
            ...
            let status = Command::new(cargo)
                ...
                .arg("wasm32-wasip2")
                .status()
                .expect("run cargo build for the animetosho plugin");
            assert!(status.success(), "animetosho plugin build failed: {status}");

            plugin_root
                .join("target/wasm32-wasip2/plugin-release/animetosho_xyz_subtitle_provider.wasm")
        })
        .clone()
}
