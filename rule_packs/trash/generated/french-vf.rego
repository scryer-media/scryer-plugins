package scryer.user.trash_guides_french_vf
import rego.v1
...
trash_detection_tables := {
  ...
  "locale_group_exact": {
    ...
    "series|thefantastics": [
      [
        "trash.locale.french.group.tier3",
        "anime"
      ]
    ],
    "series|thenightmareinhd": [
      [
        "trash.locale.french.scene",
        "any"
      ]
...
# TRASH_SCORE_SET=french-multi-vf/french-anime-multi
...
score_entry["trash_french_vostfr"] := 0 if {
    locale_intent
...
score_entry["trash_tier_1"] := 245 if {
    locale_intent
    has_fact("trash.locale.french.group.tier1")
}

score_entry["trash_tier_2"] := 240 if {
    locale_intent
    has_fact("trash.locale.french.group.tier2")
}

score_entry["trash_tier_3"] := 236 if {
    locale_intent
...
score_entry["trash_scene"] := 227 if {
    locale_intent
...
# language-not-french (radarr)
trash_lang_not_french if {
    not has_audio_language("fra")
}

score_entry["trash_lang_not_french"] := -10000 if {
    locale_intent
    trash_lang_not_french
}
