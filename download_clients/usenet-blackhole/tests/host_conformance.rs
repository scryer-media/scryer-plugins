...
fn assert_describe_returns_a_download_client_descriptor(wasm_path: &Path) {
    ...
    assert_eq!(descriptor["id"], "usenet-blackhole");
    ...
    assert_eq!(descriptor["provider"]["provider_type"], "usenet-blackhole");
...
fn plugin_wasm() -> PathBuf {
    PLUGIN_WASM
        .get_or_init(|| {
            ...
            plugin_root
                .join("target/wasm32-wasip2/plugin-release/usenet_blackhole_download_client.wasm")
        })
        .clone()
}
