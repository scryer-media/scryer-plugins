...
#[test]
fn rtorrent_release_wasm_conforms_to_the_download_client_host_contract() {
...
fn assert_describe_returns_a_download_client_descriptor(wasm_path: &Path) {
    ...
    assert_eq!(descriptor["id"], "rtorrent");
    ...
    assert_eq!(descriptor["provider"]["provider_type"], "rtorrent");
...
fn plugin_wasm() -> PathBuf {
    PLUGIN_WASM
        .get_or_init(|| {
            ...
            plugin_root.join("target/wasm32-wasip2/plugin-release/rtorrent_download_client.wasm")
        })
        .clone()
}
