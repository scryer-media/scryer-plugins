...
fn assert_describe_returns_a_download_client_descriptor(wasm_path: &Path) {
    ...
    assert_eq!(descriptor["provider"]["provider_type"], "aria2");
...
fn plugin_wasm() -> PathBuf {
    PLUGIN_WASM
        .get_or_init(|| {
            ...
            plugin_root.join("target/wasm32-wasip2/plugin-release/aria2_download_client.wasm")
        })
        .clone()
}
