...
const PLUGIN_ID: &str = "trakt";
const PROVIDER_TYPE: &str = "trakt";
const WASM_NAME: &str = "trakt_notification.wasm";

/// The configuration Scryer would have resolved for this channel.
fn scripted_config(key: &str) -> Option<String> {
    match key {
        "access_token" => Some("traktaccess".to_string()),
        _ => None,
    }
}
...
const EXPECTED_URL_PREFIX: &str = "https://api.trakt.tv/sync/collection";

static PLUGIN_WASM: OnceLock<PathBuf> = OnceLock::new();

#[test]
fn trakt_release_wasm_conforms_to_the_notification_host_contract() {
    ...
    assert_an_unknown_action_answers_empty_rather_than_trapping(&wasm_path);
    assert_a_failing_action_is_a_typed_error(&wasm_path);
...
fn assert_a_missing_required_setting_is_a_typed_error(wasm_path: &Path) {
    let script = Script {
        unset: vec!["access_token".to_string()],
        ..Script::default()
    };
    let (mut store, plugin) = instantiate(wasm_path, script);
    let result = call_notification(
    ...
    assert!(
        error.public_message.contains("access_token"),
...
fn assert_an_unknown_action_answers_empty_rather_than_trapping(wasm_path: &Path) {
    let (mut store, plugin) = instantiate(wasm_path, Script::default());
    let result = call_notification(
        &mut store,
        &plugin,
...
/// A real action missing a required parameter. Under Extism this was a
/// `FnResult` hard fault — the OAuth handshake failing took the plugin down
/// with it and the host saw a generic ABI error. It is now a typed
/// `PluginResult::Err` and the instance survives.
fn assert_a_failing_action_is_a_typed_error(wasm_path: &Path) {
    let (mut store, plugin) = instantiate(wasm_path, Script::default());
    let result = call_notification(
        &mut store,
        &plugin,
        PluginNotificationCommand::Action(PluginActionRequest {
            action: "startOAuth".to_string(),
            payload: serde_json::json!({ "query": {} }),
        }),
    );
    let PluginNotificationCommandResult::Action(PluginResult::Err(error)) = result else {
        panic!("a failing action must be a typed plugin error: {result:?}");
    };
    assert_eq!(error.code, PluginErrorCode::InvalidConfig);
}
