...
const PLUGIN_ID: &str = "sendgrid";
const PROVIDER_TYPE: &str = "sendgrid";
const WASM_NAME: &str = "sendgrid_notification.wasm";

/// The configuration Scryer would have resolved for this channel.
fn scripted_config(key: &str) -> Option<String> {
    match key {
        "from" => Some("scryer@test.invalid".to_string()),
        "recipients" => Some("ops@test.invalid".to_string()),
        "api_key" => Some("sendgridkey".to_string()),
        _ => None,
    }
}
...
const EXPECTED_URL_PREFIX: &str = "https://api.sendgrid.com/v3/mail/send";

static PLUGIN_WASM: OnceLock<PathBuf> = OnceLock::new();

#[test]
fn sendgrid_release_wasm_conforms_to_the_notification_host_contract() {
...
fn assert_a_missing_required_setting_is_a_typed_error(wasm_path: &Path) {
    ...
    assert!(
        error.public_message.contains("from"),
